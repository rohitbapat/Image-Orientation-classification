...
if train_flag=='train':
    if model_use=='nearest' or model_use=='best':
        f_name=model_use+'_model.txt'
        model_file=open(f_name,'wb')
        pickle.dump(X,model_file)
        model_file.close()
        
    if model_use=='forest':
        model_file = open('forest_model.txt', 'wb')
    ...
    if model_use=='adaboost':
        ...
        model_file=open('adaboost_model.txt','wb')
...
# Else is Testing phase
    ...
    if model_use=='adaboost':
        model_file=open('adaboost_model.txt','rb')
        ...
        model_file = open("forest_model.txt", 'rb')
